...
def overlap(type, rho_ls):
    # It only works with a w state of 2 qubit
    if type == States.W:
        overlap = np.dot(w_state_vector, np.dot(rho_ls, w_state_vector.T))[0][0]
        return np.sqrt(overlap)
    # It only works with a plus state of 1 qubit
    if type == States.Plus:
        overlap = np.dot(plus_state_vector, np.dot(rho_ls, plus_state_vector.T))
        return np.sqrt(overlap)
